...
def drop_auth_user_id_fks(cur):
    ...
    if not fks:
        ...
        return
    for fk in fks:
        stmt = f'ALTER TABLE "{fk["table_schema"]}"."{fk["table_name"]}" DROP CONSTRAINT IF EXISTS "{fk["constraint_name"]}"'
        log(f"  {stmt}")
        cur.execute(stmt)
...
def run(apply: bool) -> int:
    ...
    for col in columns:
        ...
        cur.execute(f'SELECT DISTINCT "{column}"::text AS uid FROM "{schema}"."{table}" WHERE "{column}" IS NOT NULL')
        for row in cur.fetchall():
            old_uuid = row["uid"]
            email = supabase_map.get(old_uuid)
            if not email:
                unmatched.append({"schema": schema, "table": table, "old_uuid": old_uuid, "reason": "not in auth.users"})
                continue
            new_uuid = ficium_map.get(email)
            if not new_uuid:
                unmatched.append({"schema": schema, "table": table, "old_uuid": old_uuid, "email": email, "reason": "email not in auth_portal.auth_users"})
                continue
    ...
    if skipped:
        ...
        for u in skipped: log(f"  {u}")

    if blocking:
        log(f"\n⚠️  {len(blocking)} unmatched UUID(s):")
        for u in blocking: log(f"  {u}")
        if apply:
    ...
    log(f"\n{'[DRY-RUN] ' if dry_run else ''}Plan: {total_rows} row(s) to update across {len({p['schema']+'.'+p['table'] for p in plan})} table(s)")

    if dry_run:
        for p in plan:
            log(f"  WOULD UPDATE {p['schema']}.{p['table']} SET auth_user_id={p['new_uuid']} WHERE auth_user_id={p['old_uuid']} (email={p['email']})")
        conn.rollback()
    ...
    for p in plan:
        ...
        """, (p["schema"], p["table"], p["column"], p["old_uuid"], p["new_uuid"], p["email"], False))
        cur.execute(
            f'UPDATE "{p["schema"]}"."{p["table"]}" SET "{p["column"]}" = %s WHERE "{p["column"]}" = %s',
